...
R1=$(curl -s -X POST "http://${BIND}/v1/chat/completions" \
  ...
  -d '{"model":"deepseek-chat","messages":[{"role":"user","content":"Say one word: who"}],"max_tokens":20}')
...
R2=$(curl -s -X POST "http://${BIND}/v1/chat/completions" \
  -H "Authorization: Bearer ${KEY}" \
  -H "Content-Type: application/json" \
  -d '{"model":"m-2.5","messages":[{"role":"user","content":"Say one word: who"}],"max_tokens":20}')
...
R3=$(curl -s -N -X POST "http://${BIND}/v1/chat/completions" \
  -H "Authorization: Bearer ${KEY}" \
  -H "Content-Type: application/json" \
  -d '{"model":"deepseek-chat","messages":[{"role":"user","content":"Say one word"}],"max_tokens":10,"stream":true}')
...
R4=$(curl -s -N -X POST "http://${BIND}/v1/chat/completions" \
  -H "Authorization: Bearer ${KEY}" \
  -H "Content-Type: application/json" \
  -d '{"model":"m-2.5","messages":[{"role":"user","content":"Say one word"}],"max_tokens":10,"stream":true}')
