...
- `Endpoint`（`unigateway-core/src/pool.rs:70`）已包含 `max_concurrency: Option<usize>`
- AIMD 获取逻辑（`aimd.rs:65`）已正确处理 `hard_limit`
- 静态并发上限已作为通用 endpoint capacity 原语生效
...
- 已新增 `LoadBalancingStrategy::ScoreOrdered` 枚举值（`retry.rs:5`）
- `ordered_endpoints()` 已支持该策略，保持 feedback 前置排序结果（`routing.rs:246`）
- 配置层已支持 `"score_ordered"` 映射
...
- `AttemptStartedEvent` 已包含 `active_attempts_at_start` 字段（`hooks.rs:108`）
- 执行引擎已在 attempt 启动时正确快照该指标（`chat.rs` / `embeddings.rs` / `responses.rs`）
- `StreamReport` 和 `RequestReport` 已有一致的延迟指标填充
...
| 条目 | 状态 | 理由 / 位置 |
|------|------|------|
| #1 endpoint 静态并发上限 | **已完成** | `Endpoint.max_concurrency` + AIMD integration |
| #2 `ScoreOrdered` 策略 | **已完成** | `LoadBalancingStrategy::ScoreOrdered` |
| #3 指标增强 | **已完成** | `active_attempts_at_start` in `AttemptStartedEvent` |
| #4 metadata 透传审计 | **已验证** | 无需代码改动，现有机制已覆盖 |
| #5 驱动钩子 | **暂不实施** | 现有 `on_request` + `metadata` 已满足需求 |
