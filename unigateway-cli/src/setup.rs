...
use crate::GuideCommand;
use crate::guide::{GuideParams, guide};
use crate::process::{daemonize_silent, is_running, log_path};
use crate::render::integrations::print_integrations_with_key;
use crate::setup_prompts::{
    ProviderPromptLabels, ProviderSetupInput, SetupFlow, resolve_provider_setup,
};
...
pub async fn run_guide(command: GuideCommand) -> Result<()> {
    let GuideCommand {
        ...
        api_key,
        model_mapping,
        fast_model: fast_model_arg,
        strong_model: strong_model_arg,
            ...
            7 => {
                ...
                let result = guide(
                    ...
                    GuideParams {
                        ...
                        fast_model: fast_model.as_deref().or(fast_model_arg.as_deref()),
                        strong_model: strong_model.as_deref().or(strong_model_arg.as_deref()),
                    ...
                    .unwrap()
                {
                    Some(sel) => {
                        ...
                        if let Some(pid) = already_running {
                            ...
                            println!("  Logs: {}", style(log_path().display().to_string()).dim());
