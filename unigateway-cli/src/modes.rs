...
use std::path::Path;
use unigateway_config::{GatewayState, ModeKey, ModeProvider, ModeView};
...
pub fn user_bind_address(bind: &str) -> String {
...
pub(crate) fn user_openai_base_url(bind_override: Option<&str>) -> String {
    let bind = bind_override
        ...
        .unwrap_or_else(crate::bind_from_env);
...
pub(crate) async fn load_mode_views(config_path: &str) -> Result<Vec<ModeView>> {
    ...
    Ok(state.list_mode_views().await)
}
...
pub fn effective_default_mode_id(modes: &[ModeView]) -> Option<&str> {
    modes
        .iter()
