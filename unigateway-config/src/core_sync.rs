...
pub(crate) async fn build_core_pool_for_service(
    ...
    let guard = state.read_config().await;
...
pub async fn sync_core_pools(state: &GatewayState, engine: &UniGatewayEngine) -> Result<()> {
    let file = {
        let guard = state.read_config().await;
        guard.file.clone()
    };
...
fn to_core_endpoint(
    ...
    Ok(Endpoint {
        ...
        source_endpoint_id: if provider.endpoint_id.is_empty() {
            None
        } else {
...
mod tests {
    ...
    async fn builds_core_pool_for_round_robin_service() {
        ...
        assert_eq!(
            pool.endpoints[0].provider_name.as_deref(),
            Some("moonshot-main")
        );
        assert_eq!(
            pool.endpoints[0].source_endpoint_id.as_deref(),
            Some("moonshot:global")
        );
        assert_eq!(
            pool.endpoints[0].provider_family.as_deref(),
            Some("moonshot")
        );
        assert_eq!(
            pool.endpoints[0]
